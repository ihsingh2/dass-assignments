...
SCREEN_WIDTH = 1200
SCREEN_HEIGHT = 800
SEGMENT_WIDTH = [2 * SCREEN_WIDTH // 9, 2 * SCREEN_WIDTH // 9, 2 * SCREEN_WIDTH // 9, SCREEN_WIDTH // 3]
SEGMENT_HEIGHT = 30
...
class Game:
  ...
  def init():    
    ...
    for i in range(4):
      Game.segments.append(pygame.Rect(sum(SEGMENT_WIDTH[:i]), SCREEN_HEIGHT - SEGMENT_HEIGHT, SEGMENT_WIDTH[i], SEGMENT_HEIGHT))
  ...
  def update():
    ...
    text = []
    text.append(Game.font.render(f'MOVE: {Game.moves}', True, BLACK))
    if Game.winner is None:
      text.append(Game.font.render(f'{Game.turn.name}', True, WHITE))
    else:
      text.append(Game.font.render(f'{Game.winner.name} WON!', True, WHITE))
    text.append(Game.font.render(f'CROWS CAPTURED: {Game.crows_captured}', True, BLACK))
    if Game.winner is None:
      text.append(Game.font.render(f'TIME ELAPSED: {time.strftime("%H:%M:%S", time.gmtime(time.time() - Game.start_time))}', True, BLACK))
    else:
      text.append(Game.font.render(f'TIME ELAPSED: {time.strftime("%H:%M:%S", time.gmtime(Game.finish_time - Game.start_time))}', True, BLACK))

    for i in range(len(Game.segments)):
      screen.blit(text[i], (Game.segments[i].x + (Game.segments[i].width - text[i].get_width()) // 2, Game.segments[i].y + (Game.segments[i].height - text[i].get_height()) // 2))
...
class Player(Game, pygame.sprite.Sprite):
  ...
  def vulture_blocked(self):
    ...
    if vulture_idx % 2 == 0:
      if Game.spots[(vulture_idx + 1) % 10] == None or Game.spots[(vulture_idx - 1) % 10] == None:
        return False
      if Game.spots[(vulture_idx + 3) % 10] == None or Game.spots[(vulture_idx - 3) % 10] == None:
        return False
    else:
      if Game.spots[(vulture_idx + 1) % 10] == None or Game.spots[(vulture_idx - 1) % 10] == None:
        return False
      if Game.spots[(vulture_idx + 2) % 10] == None or Game.spots[(vulture_idx - 2) % 10] == None:
        return False
      if Game.spots[(vulture_idx + 3) % 10] == None or Game.spots[(vulture_idx - 3) % 10] == None:
        return False
    return True
...
if __name__ == "__main__":
  ...
  pygame.init()
  pygame.display.set_caption('Kaoaa')
  screen = pygame.display.set_mode((SCREEN_WIDTH, SCREEN_HEIGHT))
  Game.init()

  spot_coords = [(600, 100), (667, 307), (888, 307), (708, 433), (774, 640), (600, 514), (426, 640), (492, 433), (312, 307), (532, 307)]
  ...
  while running:
    ...
    for i in range(0, len(spot_coords), 2):
      pygame.draw.line(screen, WHITE, spot_coords[i], spot_coords[(i + 4) % len(spot_coords)], width=5)
